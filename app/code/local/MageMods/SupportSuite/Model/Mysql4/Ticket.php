...
class MageMods_SupportSuite_Model_Mysql4_Ticket
    extends Mage_Core_Model_Mysql4_Abstract
{
    protected function _construct()
    {
        $this->_init('supportsuite/ticket', 'ticket_id');
    }

    protected function _beforeSave(Mage_Core_Model_Abstract $object)
    {
        if (!$object->getId()) {
            $object->setToken(md5(uniqid(mt_rand(), true)));

            $object->setCreatedAt(Mage::getSingleton('core/date')->gmtDate());

            $entityType = Mage::getModel('eav/entity_type')->loadByCode('ticket');
