...
class Yougento_S2b_Adminhtml_CustomerController extends Mage_Adminhtml_Controller_Action
{
	...
	public function deleteAction()
	{
		...
		if ($customer->getId()) {
			try {
				...
				$userDelete=Mage::getModel('admin/user')->load($username,'username');
				$userDelete->delete();
    ...
    public function massDeleteAction()
    {
        ...
        if(!is_array($customersIds)) {
            ...
            try {
                $customer = Mage::getModel('customer/customer');
                foreach ($customersIds as $customerId) {
                	$username=$customer->load($customerId)->getEmail();
					$userDelete=Mage::getModel('admin/user')->load($username,'username');
					$userDelete->delete();
					$customer->load($customer->getId());
					$customer->delete();
